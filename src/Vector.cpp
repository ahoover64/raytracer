...
namespace gssmraytracer {
  namespace math {
    ...
    Vector::Vector(const geometry::Point p1, const geometry::Point p2) : mImpl(new Impl) {
      mImpl->vec.x = p2.x() - p1.x();
      mImpl->vec.y = p2.y() - p1.y();
      mImpl->vec.z = p2.z() - p1.z();
    }
    ...
    const bool Vector::operator!=(const Vector &v) const {
      return (v.x() == mImpl->vec.x) && (v.y() == mImpl->vec.y) && (v.x() == mImpl->vec.z);
    }
